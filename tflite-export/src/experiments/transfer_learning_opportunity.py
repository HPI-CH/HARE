...
print("Recordings per subject:", recordings.count_recordings_of_subjects())
...
for model_idx, model in enumerate(models):
    ...
    for tl_sub in subs:
        ...
        recordings_tl, recordings_train = recordings.split_by_subjects([
                                                                       tl_sub])
        if len(recordings_tl) < num_folds:
            print(
                f"Skipping {tl_sub} because it has less than {num_folds} recordings")
            continue
        # setup splits
...
ax.set_title(
    f"Comparison of model accuracy with and without transfer learning on opportunity data-set\n")
