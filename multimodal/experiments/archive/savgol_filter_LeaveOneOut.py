...
def split_list_leave_person_1_out(list_to_split: list):
    test = []
    train = []
    for recording in list_to_split:
        print(recording.subject)
        if recording.subject == 1:
            test.append(recording)
        else:
            train.append(recording)
    return train,test
...
experiment_folder_path = new_saved_experiment_folder('savgol_LeavePersonOut') # create folder to store results

# TODO: save/ load preprocessed data
for recordings_tuple in recordings_to_try:
    recordings = recordings_tuple[0]
    
    # leave person 1 out for test data
    recordings_train, recordings_test = split_list_leave_person_1_out(recordings)
